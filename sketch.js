...
class Element {
  constructor(x, y, height, width, color, parent = null) {
    // (x,y) coordinate and element's width and height and color
    this.x = x;
    this.y = y;
    this.t = random(-3, 3);
    // flag determine the direction of movement
    this.flag = random(-1, 1);
    this.height = height;
    this.width = width;
    this.color = color;
    // Record the parent element of the element to prevent it from moving
    // and limit the range of movement of child elements
    this.parent = parent;
  }
  display() {
    fill(this.color);
  ...
  update() {
    if (this.parent) {
      ...
      if (this.parent.width > this.parent.height) {
        if (this.parent.flag > 0) {
          this.x += noise(this.t) * 2 - 0.5;
        } else {
          this.x -= noise(this.t) * 2 - 0.5;
        }
        if (this.x < this.parent.x) {
          this.x = this.parent.x + this.parent.width - this.width;
        } else if (this.x + this.width > this.parent.x + this.parent.width) {
        ...
        // Vertical movement
        if (this.parent.flag > 0) {
          this.y += noise(this.t) * 2 - 0.5;
        } else {
          this.y -= noise(this.t) * 2 - 0.5;
        }
        if (this.y < this.parent.y) {
          this.y = this.parent.y + this.parent.height - this.height;
        } else if (this.y + this.height > this.parent.y + this.parent.height) {
...
function draw() {
  ...
  for (let square of rParents) {
    square.display();
  }

  // Update and display child squares with Perlin Noise
  for (let i = 0; i < rChildren.length; i++) {
    let square = rChildren[i];
    square.update();
    square.display();
    if (square.t <= 0) {
      square.t -= random(0.01, 0.02);
    } else {
      square.t += random(0.01, 0.02);
    }
  }
